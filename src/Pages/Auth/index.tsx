...
import { ColorPicker, Flex, ConfigProvider, theme } from 'antd';
import { SunOutlined, MoonOutlined, SafetyCertificateOutlined } from '@ant-design/icons';
import { useAppDispatch, useAppSelector } from '@/Store/hooks';
import { setToggleTheme, setPrimaryColor } from '@/Store';

const AuthLayout: React.FC = () => {
  const dispatch = useAppDispatch();
  const { isToggleTheme, primaryColor } = useAppSelector((state) => state.layout);
  ...
  const handlePrimaryColorChange = (_: any, hex: string) => {
    dispatch(setPrimaryColor(hex));
  };
  const antTheme = {
  ...
  return (
    <ConfigProvider theme={antTheme}>
      <div className={`universal-container ${isDark ? 'dark' : ''}`} style={{ '--color-primary': primaryColor } as React.CSSProperties}>
        ...
        <section className="universal-form-side">
          <Flex className="controls-area" gap="middle" align="center">
            <ColorPicker value={primaryColor} onChange={handlePrimaryColorChange} />
