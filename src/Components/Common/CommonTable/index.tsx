...
export default function CommonTable<T extends object>({ columns, data, loading, total, current = 1, pageSize = 10, onTableChange, onSearch, searchPlaceholder = "Search...", onAdd, isActive, onActiveChange, onExportAll, fileName = "report", title, companyName, email, }: CommonTableProps<T>) {
  ...
  const filteredColumns = useMemo(() => columns.filter((c) => visibleCols.includes(String(c.dataIndex))),
    [columns, visibleCols]
  );
  ...
  return (
    <div className="common-table">
      ...
      <Table className="common-table-surface" rowKey={(r: any) => r.id} columns={filteredColumns as any} dataSource={data} loading={loading} pagination={{ current, pageSize, total, showSizeChanger: true, }} onChange={onTableChange} locale={{ emptyText: "No Data Found" }} />
    </div>
  );
}